...
function Dashboard() {
  ...
  const fetchTodayMoodData = async (userEmail) => {
    if (!userEmail) return;
    try {
      const response = await fetch(`${API_BASE_URL}/api/moods?userId=${userEmail}`);
  ...
  const fetchReflections = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reflections?userId=${idToUse}`);
...
const DailySparkPanel = ({ userId, onSeeMore }) => {
  ...
  useEffect(() => {
    if (!userId) return;
    fetch(`${API_BASE_URL}/api/recommendations?mood=neutral&userId=${userId}`)
